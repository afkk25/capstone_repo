...
def aggregate_commune_summary(origins: pd.DataFrame) -> pd.DataFrame:
    ...
    for (commune_id, commune_name, district_name), group in df.groupby(["commune_id", "commune_name", "district_name"], dropna=False):
        ...
        for pct_key in ["pct_pop_access_30min", "pct_pop_access_60min", "pct_pop_access_threshold", "pct_pop_score_below_50", "unreachable_population"]:
            if row.get(pct_key) is not None:
                row[pct_key] = float(row[pct_key]) * 100.0
        rows.append(row)
...
def compute_kpis(origins: pd.DataFrame | None, facilities: pd.DataFrame | None, stops: pd.DataFrame | None, facilities_near_transit: int | None) -> dict:
    ...
    if kpis["pct_population_within_30_min"] is not None:
        kpis["pct_population_within_30_min"] = float(kpis["pct_population_within_30_min"]) * 100.0
    if kpis["pct_population_within_60_min"] is not None:
        kpis["pct_population_within_60_min"] = float(kpis["pct_population_within_60_min"]) * 100.0
    if kpis["coverage_gap_pct"] is not None:
        kpis["coverage_gap_pct"] = float(kpis["coverage_gap_pct"]) * 100.0

    total_population = float(kpis["population"] or 0.0)
    within_60_pct = float(kpis["pct_population_within_60_min"] or 0.0)
    within_60_population = total_population * within_60_pct / 100.0
    gap_pct = float(kpis["coverage_gap_pct"] or 0.0)
    gap_population = total_population * gap_pct / 100.0
    logger.info(
        "KPI consistency total_population=%.0f within60_population=%.0f within60_pct=%.2f below50_population=%.0f coverage_gap_pct=%.2f",
        total_population,
        within_60_population,
        within_60_pct,
        gap_population,
        gap_pct,
    )
