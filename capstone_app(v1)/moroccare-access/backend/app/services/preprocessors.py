...
def preprocess_origins(df: pd.DataFrame) -> gpd.GeoDataFrame:
    ...
    # Canonical score scale is 0-100 in the active app stack.
    if "accessibility_score" in gdf.columns:
        score_non_null = gdf["accessibility_score"].dropna()
        if not score_non_null.empty and float(score_non_null.max()) <= 1.5:
            gdf["accessibility_score"] = gdf["accessibility_score"] * 100.0

    # If score is absent but travel time exists, derive the canonical 0-100 score.
    # score = 100 * (1 - min(tt / 45, 1))
    if "accessibility_score" in gdf.columns and gdf["accessibility_score"].isna().all() and "total_travel_time_min" in gdf.columns:
        tt = pd.to_numeric(gdf["total_travel_time_min"], errors="coerce")
        gdf["accessibility_score"] = 100.0 * (1.0 - np.minimum(tt / 45.0, 1.0))
        gdf["accessibility_score"] = gdf["accessibility_score"].clip(lower=0.0, upper=100.0)
