...
def load_notebook_origin_metrics(city_id: str) -> pd.DataFrame:
    ...
    if df is not None:
        if "city_id" in df.columns:
            ...
            return city_rows
        # City-folder files are already scoped by path and do not require city_id.
        if source_path is not None and source_path.resolve().parent == paths.root.resolve():
            return df
