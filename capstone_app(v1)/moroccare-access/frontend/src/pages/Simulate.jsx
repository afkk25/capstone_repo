...
function normalizeScorePercentValue(value) {
  const numeric = Number(value);
...
function summarizeRows(rows, mode = "accessibility") {
  ...
  const underservedPopulation = safeRows.reduce((sum, row) => {
    ...
    if (!Number.isFinite(score) || score >= 50) return sum;
...
function getSimulationDefaults(city) {
  ...
  return {
    ...
    max_travel_time_min: Number.isFinite(Number(defaults.max_travel_time_min)) ? Number(defaults.max_travel_time_min) : 45,
    walking_distance_m: Number.isFinite(Number(defaults.walking_distance_m)) ? Number(defaults.walking_distance_m) : 600,
    transport_speed_kmh: Number.isFinite(Number(defaults.transport_speed_kmh)) ? Number(defaults.transport_speed_kmh) : 20
  };
}
...
function buildPayload({ selectedIntervention, placement, mode, advancedSettings, cityDefaults }) {
  ...
  const payload = {
    ...
    transport_speed_kmh: Number(advancedSettings.transport_speed_kmh || cityDefaults.transport_speed_kmh || 20),
    max_travel_time_min: Number(advancedSettings.max_travel_time_min || cityDefaults.max_travel_time_min || 45),
    facility_locations: [],
  ...
  if (selectedIntervention.placementTarget === "facility_locations") {
    ...
    payload.scenario_type = "add_facility";
    payload.location = location;
  } else {
    payload.transport_stop_locations = [location];
    payload.scenario_type = "add_stop";
    payload.location = location;
  }
  return payload;
}
...
export default function Simulate({
  ...
  useEffect(() => {
    if (!import.meta.env.DEV || !payload) return;
  ...
  const simulationDeltaSummary = simulationResult?.delta_summary || {};
  ...
  const baselineUnderservedPopulation = simulationOrigins
    .filter((origin) => normalizeScoreValue(origin.baseline_score ?? origin.before_score) < 50)
    .reduce((sum, origin) => sum + (Number(origin.population) || 0), 0);
  const simulatedUnderservedPopulation = simulationOrigins
    .filter((origin) => normalizeScoreValue(origin.simulated_score ?? origin.after_score ?? origin.accessibility_score) < 50)
    .reduce((sum, origin) => sum + (Number(origin.population) || 0), 0);
  ...
  const maxTravelTime = Number(advancedSettings.max_travel_time_min || 45);
  ...
  const fmtPp = (value) => `${value >= 0 ? "+" : ""}${Number(value).toFixed(1)} pp`;
  ...
  const topImprovedCommunes = (Array.isArray(simulationResult?.commune_impacts) ? simulationResult.commune_impacts : [])
    .filter((row) => Number.isFinite(Number(row.score_gain)) && Number(row.score_gain) > 0)
    .sort((a, b) => Number(b.score_gain) - Number(a.score_gain))
    .slice(0, 3)
    .map((row) => String(row.commune_name || row.district_name || ""))
    .filter(Boolean)
    .join(", ");
  const contextRows = [
    placement
    ...
    hasResult && featureChangedOriginCount > 0
      ...
      : null,
    hasResult && topImprovedCommunes
      ? {
          label: "Best improved communes",
          value: topImprovedCommunes,
          detail: null,
          variant: "stacked"
        }
      : null
  ...
  const populationImproved = toFiniteOrNull(simulationSummary.population_improved ?? simulationDeltaSummary.improved_population);
  const newlyCovered60 = toFiniteOrNull(simulationSummary.newly_covered_population_60min);
  const averageTravelTimeReduction = toFiniteOrNull(simulationSummary.average_travel_time_reduction_min);
  const averageAccessibilityScoreGain = normalizeScorePercentValue(
    simulationSummary.average_accessibility_score_gain ?? simulationDeltaSummary.avg_accessibility_delta
  );
  const hasQuantitativeMetrics =
    hasResult &&
    [populationImproved, newlyCovered60, averageTravelTimeReduction, averageAccessibilityScoreGain].some((value) => value !== null);
  const showPreviewModeNotice = Boolean(placement) && (simulationError || (hasResult && !hasQuantitativeMetrics) || (runAttempted && !simulationPending && !hasResult));
  ...
  const insightMetrics = [
    {
      ...
      value: hasResult ? formatCount(populationImproved, unavailableText) : unavailableText,
      delta: null
    },
    {
      emphasis: "primary",
      label: "Average time saved",
      value: hasResult ? (averageTravelTimeReduction !== null ? fmtTime(Math.max(0, averageTravelTimeReduction)) : unavailableText) : unavailableText,
      delta: null
    },
    {
      emphasis: "secondary",
      label: "Accessibility score gain",
      value: hasResult ? (averageAccessibilityScoreGain !== null ? fmtPp(Math.max(0, averageAccessibilityScoreGain)) : unavailableText) : unavailableText,
      delta: hasResult ? `Before ${fmtPct(accessBefore)} -> After ${fmtPct(accessAfter)}` : null
    },
    {
      emphasis: "secondary",
      label: "Newly covered within 60 min",
      value: hasResult ? formatCount(newlyCovered60, unavailableText) : unavailableText,
      delta: hasResult ? `Threshold ${maxTravelTime} min: ${scenarioAccessibleCount.toLocaleString()} areas` : null
    }
  ];
  const visibleInterventions = interventionType
    ...
    : interventionOptions;
  useEffect(() => {
    if (!import.meta.env.DEV || !simulationResult) return;
    console.log("Simulation raw response", simulationResult);
    console.log("Normalized simulation result", {
      populationImproved,
      newlyCovered60,
      averageTravelTimeReduction,
      averageAccessibilityScoreGain,
      accessBefore,
      accessAfter,
      timeBefore,
      timeAfter
    });
  }, [simulationResult, populationImproved, newlyCovered60, averageTravelTimeReduction, averageAccessibilityScoreGain, accessBefore, accessAfter, timeBefore, timeAfter]);
            ...
            <section className={`simflow-panel simflow-panel--status simflow-panel--summary is-${scenarioStatus.tone}`}>
              ...
              {workspaceStep === 3 ? (
                <div className="simflow-actions">
                  <button
                    type="button"
                    className="simflow-text-action simflow-text-action--primary"
                    disabled={!canRunSimulation || simulationPending}
                    onClick={runScenario}
                  >
                    {simulationPending ? t("simflow.running") : "Run scenario"}
                  </button>
              ...
              {simulationPending ? (
                ...
                <div className="simflow-preview-note" role="status">
                  Scenario preview mode: placement is shown on the map. Quantitative comparison is still being finalized.
                </div>
              ) : (
