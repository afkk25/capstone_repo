...
"""
This is not the main accessibility simulation.
It only modifies ML feature columns and should not power the map simulation.
"""
...
def legacy_ml_proxy_simulation(
    features_df: pd.DataFrame,
    scenario: dict,
    baseline_scores: np.ndarray | None = None,
) -> tuple[pd.DataFrame, dict[str, list[dict[str, float | str]]]]:
    """Backward-compatible alias for legacy feature-proxy simulation."""
    return apply_intervention(features_df, scenario, baseline_scores=baseline_scores)
