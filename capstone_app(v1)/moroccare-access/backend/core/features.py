...
def compute_facility_proxy_features(
    ...
    """
    Legacy ML fallback only.
    Facility proxy features must not be used as origin demand points in the main app.
    """
    if health_gdf.empty:
        return pd.DataFrame()
...
def compute_features(health_gdf: gpd.GeoDataFrame, stops_gdf: gpd.GeoDataFrame, city_config: dict) -> pd.DataFrame:
    ...
    Facility proxy features are legacy ML fallback only and must not be used as origin demand points.
    """
